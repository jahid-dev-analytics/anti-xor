/* The procedural car that the turntable frames are rendered FROM.
   This is build-time source, not shipped: the page loads pre-rendered WebP
   frames instead, so none of this WebGL reaches a visitor. It was lifted out
   of public/js/scene.js unchanged when the hero moved to a frame sequence.
   tools/turntable/render.mjs wraps it in a harness and drives draw().

   It expects clamp(), smooth(), mobileQ and a <canvas id="gl"> from its host. */
